...
export default function AdminMallOrdersView() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'paid': return <span className="px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-emerald-500/10 text-emerald-500 border border-emerald-500/20">Paid</span>;
      case 'shipped': return <span className="px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-blue-500/10 text-blue-500 border border-blue-500/20">Shipped</span>;
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400">Manage C2G Mall purchases.</p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-zinc-800">
              {loading ? (
                <tr><td colSpan={5} className="p-8 text-center text-zinc-500">Loading mall orders...</td></tr>
              ) : filteredOrders.length === 0 ? (
                <tr><td colSpan={5} className="p-8 text-center text-zinc-500">No mall orders found.</td></tr>
              ) : (
                  ...
                  <tr key={order.id} className="hover:bg-zinc-800/50 transition-colors group">
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button className="p-2 text-indigo-400 hover:text-white hover:bg-indigo-500/20 rounded-lg transition-colors" title="Edit Status">
