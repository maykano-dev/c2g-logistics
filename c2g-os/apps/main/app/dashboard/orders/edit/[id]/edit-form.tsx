...
import { updateLinkOrder } from "../../actions";
import { useRouter } from "next/navigation";
import { useModal } from "@/components/providers/modal-provider";

export function EditLinkOrderForm({ 
  order,
  exchangeRate, 
  ...
  // Multi-item State initialized from existing order
  const [items, setItems] = useState<any[]>(() => {
    if (order.items && Array.isArray(order.items)) {
      return order.items;
    }
    // Fallback if it's an old order format
    return [{ 
      id: order.id, 
      product_link: order.product_link || '', 
      cny_price: order.cny_price || 0, 
  ...
  const [itemPreviews, setItemPreviews] = useState<Record<string, string>>(() => {
    ...
    if (order.items && Array.isArray(order.items)) {
      order.items.forEach((item: any) => {
        if (item.screenshot_url) previews[item.id] = item.screenshot_url;
      });
    } else if (order.screenshot_url) {
      previews[order.id] = order.screenshot_url;
    }
    return previews;
  });
  
  const [shippingMode, setShippingMode] = useState<string>(order.shipping_mode || '');
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  
  const updateAction = updateLinkOrder.bind(null, order.id);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    items.forEach((item, index) => {
      ...
      // If it has no preview and no new file selected, it's missing a screenshot
      if (!itemPreviews[item.id] && !itemFileNames[item.id]) { newErrors[`screenshot_${item.id}`] = "Screenshot is mandatory for this item"; hasError = true; }
    });
            ...
            {items.map((item, index) => (
              <div key={item.id} className="glass-panel p-6 md:p-8 space-y-6 relative border border-border/50">
                ...
                <div className="space-y-3 mt-6 pt-6 border-t border-border/50">
                  <label className="text-sm font-semibold flex items-center gap-2">
                    Item Screenshot <span className="text-amber-500">(Leave blank to keep existing)</span>
                  </label>
                  <div className="grid grid-cols-2 gap-4 h-32">
                    ...
                    <div className={`relative border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center bg-secondary/10 hover:bg-secondary/20 transition-colors cursor-pointer group h-full ${
                      ...
                      <p className="text-xs font-medium text-center text-muted-foreground px-2 line-clamp-1">
                        {itemFileNames[item.id] || "Upload new"}
                      </p>
                    </div>
                ...
                >
                  <div className="flex items-center gap-2">
                    {shippingMode === 'express' ? <><Zap className="w-4 h-4 text-orange-500" /> Air Express</> : 
                     ...
                     <span className="text-muted-foreground capitalize">{shippingMode.replace('_', ' ')}</span>}
                  </div>
