...
import { AGENT_NAV_GROUPS } from './agent-navigation';
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="p-6 border-b border-zinc-800 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white">C2G COS</h1>
          <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-semibold">Customer Service</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-6 px-4 space-y-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {AGENT_NAV_GROUPS.map((group, index) => (
          <div key={index} className="space-y-2">
            ...
            <div className="space-y-1">
              {group.items.map((link) => {
                const isActive = pathname === link.href || (pathname.startsWith(link.href) && link.href !== '/agent/dashboard');
