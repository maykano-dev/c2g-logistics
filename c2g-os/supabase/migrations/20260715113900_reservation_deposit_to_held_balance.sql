-- Update reservation deposit RPC to move funds to held_balance instead of permanently deducting
-- This implements the escrow pattern: available_balance -> held_balance -> settled on invoice
...
BEGIN
  ...
  -- 2. Lock the wallet
  SELECT available_balance, COALESCE(held_balance, 0) INTO v_current_balance, v_current_held
  FROM public.wallets
  ...
  -- 3. Move funds from available_balance to held_balance (escrow)
  UPDATE public.wallets
  SET available_balance = v_current_balance - p_deposit_amount,
      held_balance = v_current_held + p_deposit_amount
  WHERE id = p_wallet_id;

  -- 4. Record wallet transaction as a hold (not a permanent deduction)
  INSERT INTO public.wallet_transactions (
    ...
    'completed',
    'Shipping deposit held for Reservation ' || p_reservation_id,
    p_reservation_id
  );
  ...
  RETURN jsonb_build_object('success', true, 'new_balance', v_current_balance - p_deposit_amount);
END;
$$;
